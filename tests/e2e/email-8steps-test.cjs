...
async function runEmail11StepsTest() {
  ...
  try {
    ...
    const emailButton = await page.locator('[data-testid="tool-selector"] button[data-id="send_email"]');
    ...
    const modalSaveButton = await page.locator('button:has-text("Save")').last(); // Get the last Save button (modal one)
    ...
    const calcButton = await page.locator('[data-testid="tool-selector"] button[data-id="calculate"]');
    ...
    const toolCountAfterCalc = await page.locator('[data-testid*="rf__node"]').count();
    ...
    const zoomOutButton = await page.locator('button[title*="zoom out"], button[aria-label*="zoom out"]').first();
    ...
    if (toolNodes.length >= 2) {
      ...
      if (calcBox) {
        ...
        await page.mouse.move(calcBox.x + calcBox.width / 2, calcBox.y + calcBox.height / 2);
        await page.mouse.down();
        // Drag much further - 400px right and 300px down
        await page.mouse.move(calcBox.x + 400, calcBox.y + 300);
        await page.mouse.up();
        console.log(`🔍 Dragged to: x=${calcBox.x + 400}, y=${calcBox.y + 300}`);
    ...
    const exportButton = await page.locator('button:has-text("Export"), button[title*="export"], button[aria-label*="export"]').first();
    ...
    if (exportExists) {
      ...
      const menuButton = await page.locator('button[aria-label*="menu"], button[title*="menu"]').first();
      ...
      if (menuExists) {
        ...
        const exportInMenu = await page.locator('button:has-text("Export"), a:has-text("Export")').first();
    ...
    const isRemoveButtonFocused = await removeButton.evaluate(el => el === document.activeElement);
